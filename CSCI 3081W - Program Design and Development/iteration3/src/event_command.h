...
class EventCommand : public EventBaseClass {
 public:
  /**
   * @brief Constructs the event with a given user-specified command.
   *
   * @param cmd The command inputted by the user (e.g. speed up or slow down)
   */
  explicit EventCommand(enum event_commands cmd) : cmd_(cmd) {}

  void EmitMessage(void) { printf("Motion cmd %d received\n", cmd_); }
  /**
   * @brief Returns the command (e.g. speed up or slow down).
   */
