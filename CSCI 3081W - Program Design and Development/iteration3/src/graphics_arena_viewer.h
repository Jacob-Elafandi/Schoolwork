 ...
 *
 *  After constructing a new GraphicsArenaViewer, call Run() to start and run
 *  the application.  Run() will not return until the application window is
 *  closed. Make sure that you call cs3081::InitGraphics() before creating the
 *  GraphicsArenaViewer app.  Example:
 *
 *  ```
 *  int main(int argc, char **argv) {
 *      cs3081::InitGraphics();
 *      GraphicsArenaViewer *app = new GraphicsArenaViewer();
 *      app->Run();
...
class GraphicsArenaViewer : public GraphicsApp {
 public:
  /**
   * @brief Creates a new graphical window and constructs a new Arena
   * to be displayed in it.
   *
   * @param params The parameters with which the new Arena will be constructed.
   */
  ...
  /**
   * @brief Returns a pointer to the arena.
   */
  Arena* get_arena(void) const { return arena_; }

 private:
  /**
   * @brief Draw the player in the arena using nanogui.
   *
  ...
  /**
   * @brief Draw the player's battery level.
   *
   * This function requires an active nanovg drawing context (ctx), so it should
   * probably only be called from with \ref DrawUsingNanoVG().
   *
   ...
   * @param player The player handle.
   */
  void DrawPlayerBattery(NVGcontext *ctx, const class Player* const player);
   ...
   *
   * @param ctx The nanogui context.
   * @param home The HomeBase handle.
   */
   ...
   *
   * @param ctx The nanogui context.
   * @param sensor The handle of the sensor.
   * @param robot The handle of the robot that the sensor belongs to.
   */
  void DrawRobotSensors(NVGcontext *ctx,
    const class SensorProximity* const sensor, const class Robot* const robot);
